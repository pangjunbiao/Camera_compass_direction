...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    model = MaskRCNN(backbone, num_classes=num_classes)    #创建模型实例，Mask R-CNN 是一种基于深度学习的目标检测模型，能够同时进行物体检测和语义分割
...
def main(args):
    ...
    # train_dataset = read_data(data_root, "train", data_transform["train"])
    train_sampler = None
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mean_loss, lr = utils.train_one_epoch(model, optimizer, train_data_loader,           #标记一下
        ...
        with open(det_results_file, "a") as f:
            ...
            # result_info = [f [mean_loss.item()]] + [f"{lr:.6f}"]
        ...
        with open(seg_results_file, "a") as f:
            ...
            txt = "epoch:{} {}".format(epoch, '  '.join(result_info))
            f.write(txt + "\n")
        val_pre.append(np.mean(seg_info))
    ...
    # plot mAP curve
    if len(val_map) != 0:
        from plot_curve import plot_map,plot_map_point
        plot_map(val_map)
        plot_map_point(val_pre)
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录E:\BJL_毕业存档\02摄像头朝向\02数据\data   'D:\\ISS\\mask_RCNN\\mask_rcnn\\data\\coco2017'
    parser.add_argument('--data-path', default='E:\\Camera_compass_direction\\02Camera_direction\\02数据\\data\\coco2017', help='dataset')
    # 检测目标类别数(不包含背景)
    ...
    # parser.add_argument('--resume', default='D:\ISS\mask_RCNN\mask_rcnn\save_weights\model_0.pth', type=str, help='resume from checkpoint')
    # 指定接着从哪个epoch数开始训练
    ...
    parser.add_argument('--lr-steps', default=[30,40], nargs='+', type=int,
