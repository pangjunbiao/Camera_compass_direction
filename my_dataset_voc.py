...
class VOCInstances(Dataset):
    ...
    def __init__(self, voc_root, year="2012", txt_name: str = "train.txt", transforms=None):
        ...
        for idx, (img_path, xml_path, mask_path) in enumerate(zip(images_path, xmls_path, masks_path)):
            ...
            instance_point = self.get_point(instances_mask)
            ...
            self.point.append(instance_point)
    ...
    def get_point(self,instance_mask):
        '''
        :param instance_mask: 输入掩码图
        :return: 输出掩码中心点
        '''
        mask = instance_mask
        mask_dict = {}  # 存贮每个目标的所有坐标
        for i in range(np.shape(mask)[0]):
            for j in range(np.shape(mask)[1]):
                if mask[i][j] != 0:
                    if mask[i][j] not in mask_dict.keys():
                        mask_dict[mask[i][j]] = []
                    mask_dict[mask[i][j]].append([i, j])
        label_point=[]
        for cls in mask_dict.keys():
            temp = np.array(mask_dict[cls])
            mean_row = int(np.mean(temp[:, 1]))
            mean_column = int(np.mean(temp[:, 0]))
            label_point.append([mean_column,mean_row])
        return label_point
