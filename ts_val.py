...
def main(args):
    ...
    if args.resume:
        ...
        det_info, seg_info ,key_point_info_1,key_point_info_2= utils.evaluate(model, val_data_loader, device=device,type="val")
        # print(det_info)  # pascal mAP of boxs
        print("点检测结果：",np.mean(seg_info))
        print("object_map:",np.mean(key_point_info_1))
        print("shadow_map",np.mean(key_point_info_2))
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--resume', default='.\save_weights\\4_5model_onlylight.pth', type=str, help='resume from checkpoint') #载入只光源方向
    parser.add_argument('--resume', default='.\save_weights\\model_78.pth', type=str, help='resume from checkpoint') #包含含光源方向和iou
    # parser.add_argument('--resume', default='.\save_weights\\14new_model.pth', type=str, help='resume from checkpoint') #载入只有IOU
    ...
    parser.add_argument('--lr-steps', default=[60,65], nargs='+', type=int,
