...
'''
在faster_rcnn_framework.py中进行修改，对应不同的loss
'''
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets,light_targets):
    # type: (Tensor, Tensor, List[Tensor], List[Tensor]) -> Tuple[Tensor, Tensor,Tensor]
    """
    ...
    def light_loss(box_reg, labels_pos, light):                       #标记一下  box_regs输入
        #计算 光源loss
        light_loss = 0
        ...
        for i in object_index:
            n=0
            light_list = torch.zeros(len(shadow_index)).cuda()
            for j in shadow_index:
                if torch.abs(
                        torch.mean(box_reg[i] - box_reg[j]) / torch.max(torch.sum(box_reg[i][0::2] - box_reg[j][0::2]),
                                                                        torch.sum(box_reg[i][1::2] - box_reg[j][
                                                                                                      1::2]))) < 1:
                    light_list[n] = det_utils.smooth_l1_loss(
                        torch.atan2(box_reg[i][0:2] + box_reg[i][2:], box_reg[j][0:2] + box_reg[j][2:]), light)
                    n += 1
            if light_list != []:
                if n <len(shadow_index):
                    light_loss += 0.1*torch.min(light_list[0:j+1])
                else:
                    light_loss += 0.1*torch.min(light_list[0:j])
        #print('\n test:',light_loss)
        light_loss=light_loss/len(object_index)
        return light_loss
    """

    def light_loss(box_reg, labels_pos, light):
        ...
        # 找到目标对象和阴影的索引
        for i in range(len(labels_pos)):
            if labels_pos[i][0] == 1:  # 1 表示目标对象
                object_index.append(i)
            elif labels_pos[i][0] == 2:  # 2 表示阴影
                shadow_index.append(i)

        # 遍历每个目标对象
        for i in object_index:
            light_list = torch.zeros(len(shadow_index)).cuda()  # 存储光源差异的临时列表
            n = 0  # 记录有效阴影数

            # 遍历每个阴影
            for idx, j in enumerate(shadow_index):
                # 计算目标对象与阴影之间的特征差异
                diff = torch.abs(
                    torch.mean(box_reg[i] - box_reg[j]) /
                    torch.max(
                        torch.sum(box_reg[i][0::2] - box_reg[j][0::2]),  # x 坐标差异
                        torch.sum(box_reg[i][1::2] - box_reg[j][1::2])  # y 坐标差异
                    )
                )
                # 如果差异小于阈值，则计算光源差异
                if diff < 1:
                    light_list[n] = det_utils.smooth_l1_loss(
                        torch.atan2(box_reg[i][0:2] + box_reg[i][2:], box_reg[j][0:2] + box_reg[j][2:]),
                        light
                    )
                    n += 1

            # 计算 light_loss，确保 light_list 不为空
            if n > 0:  # 至少有一个有效值
                light_loss += 0.1 * torch.min(light_list[:n])
        print('/n',light_loss)
...
"""
    ...
    label_pos=labels_pos.reshape((len(labels_pos),1))
    box_reg = box_regression[sampled_pos_inds_subset, labels_pos]
    # 计算光源方向loss
    light_loss = light_loss(box_reg,label_pos,light_targets)
    ...
    return classification_loss, box_loss,light_loss
...
def gaussian_kernel(size_w, size_h, center_x, center_y,sigma=2.0):
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    criterion = torch.nn.MSELoss().cuda()#改为mse模型
    ...
    for temp in b:
        index = np.max(temp)
        a = np.argwhere(temp==index)[0]
        c=gaussian_kernel(28,28,a[0],a[1])
        c[c > 1] = 1
        c[c < 0.01] = 0
        heatmap.append(c)
    mask_targets=torch.from_numpy(np.array(heatmap)).float()
    mask_targets=mask_targets.cuda()


    # mask_targets.cpu().numpy()
    mask_loss = 2*criterion(
...
class RoIHeads(torch.nn.Module):
    ...
    def mean_light(self,
        ...
        mean_light = torch.zeros(num_images)
        for img_id in range(num_images):
            light_list = []
            num = 0
            labels = target[img_id]["labels"]
            lights = target[img_id]["light"]
            for i in range(len(labels)):
                if labels[i] == 1:
                    num += 1
                    light_list.append(lights[i])
            mean_light[img_id] = light_list[num//2]
        return mean_light.cuda()
    #
    ...
    def forward(self,
        ...
        if self.training:
            # 划分正负样本，统计对应gt的标签以及边界框回归信息
            light_targets = self.mean_light(targets)
            proposals, matched_idxs, labels, regression_targets = self.select_training_samples(proposals, targets)
            # a=np.array(regression_targets[0].cpu())
            # print(a)
        else:
            labels = None
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg,light_loss = fastrcnn_loss(
                class_logits, box_regression, labels, regression_targets,light_targets)
            losses = {
                "loss_classifier": loss_classifier,
                "loss_box_reg": loss_box_reg+0.001*light_loss
            }
        else:
