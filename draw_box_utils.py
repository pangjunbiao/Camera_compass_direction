...
def draw_point_text(draw,
    ...
    if cls == 1:
        ...
        class_name="shadow"
    left, top, right, bottom = x,y,x+10,y+10
    display_str = f"{class_name}: {int(x)},{int(y)}"
    display_str_heights = [font.getsize(ds)[1] for ds in display_str]
    # # Each display_str has a top and bottom margin of 0.05x.
    display_str_height = (1 + 2 * 0.05) * max(display_str_heights)
...
def draw_masks(image, masks, colors,thresh: float = 0.3, alpha: float = 0.5 ):
...
def draw_objs(image: Image,
    ...
    c1,x_1,y_1 = points[0]
    c2,x_2, y_2 = points[1]
    draw.line([(y_1,x_1),(y_2,x_2)],width=6,fill=(255,255,0))
    # if draw_masks_on_image and (masks is not None):
        # Draw all mask onto image.画所有关键点
        # image = draw_masks(image, masks, colors,mask_thresh)
        #绘制标注
        # draw = ImageDraw.Draw(image)
        # for mask, color, point, box in zip(masks, colors, points, boxes):
        #     draw_point_text(draw, point[1:], point[0], color,box)
