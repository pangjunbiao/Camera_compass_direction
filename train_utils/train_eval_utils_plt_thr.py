...
def evaluate(model, data_loader, device):
    ...
    for thr in thr_list:
        ...
        for image, targets in metric_logger.log_every(data_loader, 100, header):
            ...
            seg_info.extend(key_metric.prepare_for_coco_segmentation(targets, outputs,thr))
            # key_metric.update(targets, outputs)
            # metric_logger.update(model_time=model_time)
        seg_list.append(np.mean(seg_info))
    # gather the stats from all processes
    df = pd.DataFrame(np.array(seg_list))
    df.to_csv('D:/ISS/mask_RCNN/mask_rcnn/result/from_99model_point_map.csv')  # 存储到CSV中
    plt.plot(thr_list,np.array(seg_list))
    plt.show()
